...
class ImageQuerier:
    def __init__(self, index: faiss.Index, metadata: list, u2net_model_path: str, augmentation_map_path: str):
        ...
        with open(augmentation_map_path, "r") as f:
            self.augmentation_map = json.load(f)

        self.reverse_map = {}
        for original, aug_list in self.augmentation_map.items():
            for aug in aug_list:
                self.reverse_map[aug] = original
            self.reverse_map[original] = original

    def query(self, image_path: str, top_k: int = 5, faiss_k: int = 10000) -> List[Dict]:
    ...
    def _aggregate_results(self, results: List[Tuple[str, float]]) -> List[Dict]:
