...
class ImageAugmenter:
    ...
    def augment_image_bytes(self, image_bytes: bytes, base_filename: str) -> Dict[str, bytes]:
        ...
        filename = f"{Path(base_filename).stem}_original.png"
        results[filename] = to_bytes(np.dstack([rgb, alpha]))
        ...
        for i in range(self.augmentations_per_image):
            ...
            filename = f"{Path(base_filename).stem}_aug_{i:03d}.png"
            results[filename] = to_bytes(aug_rgba)
