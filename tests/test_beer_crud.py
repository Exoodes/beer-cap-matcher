...
from src.db.crud.beer_crud import (
    create_beer,
    delete_beer,
    get_all_beers,
    get_beer_by_id,
...
class TestBeerCRUD:
    ...
    async def test_create_beer(self, db_session: AsyncSession):
        ...
        beer = await create_beer(
            ...
            beer_brand_id=self.beer_brand.id,
        )
    ...
    async def test_get_beer_by_id(self, db_session: AsyncSession):
        ...
        fetched_beer = await get_beer_by_id(
            db_session, created_beer.id, load_country=True
        )
    ...
    async def test_update_beer(self, db_session: AsyncSession):
        country_original = await create_country(
            db_session, CountryCreateSchema(name="Old Country")
        )
        ...
        beer = await create_beer(
            ...
            "Old Name",
            rating=2,
            beer_brand_id=beer_brand_original.id,
            country_id=country_original.id,
        )

        country_new = await create_country(
            db_session, CountryCreateSchema(name="New Country")
        )
        ...
        update_data = BeerUpdateSchema(
            ...
            beer_brand_id=beer_brand_new.id,
        )
        updated = await update_beer(
            db_session, beer.id, update_data, load_country=True, load_beer_brand=True
        )
    ...
    async def test_delete_non_existent_beer_does_nothing(
