...
class TestBeerCRUD:
    ...
    async def test_create_beer(self, db_session: AsyncSession):
        beer = await create_beer(db_session, "Test Beer", self.beer_brand.id)
    ...
    async def test_get_beer_by_id(self, db_session: AsyncSession):
        created_beer = await create_beer(db_session, "ID Test Beer", self.beer_brand.id)
    ...
    async def test_get_all_beers(self, db_session: AsyncSession):
        await create_beer(db_session, "Beer One", self.beer_brand.id)
        await create_beer(db_session, "Beer Two", self.beer_brand.id)
    ...
    async def test_delete_beer(self, db_session: AsyncSession):
        created_beer = await create_beer(db_session, "Delete Test Beer", self.beer_brand.id)
